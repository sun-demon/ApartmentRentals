...
class RoomsNumberDialog(
    context: Context
) : Dialog(context) {
    private lateinit var binding: DialogRoomsNumberBinding
    private lateinit var range: Range<Int>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        requestWindowFeature(Window.FEATURE_NO_TITLE)
        binding = DialogRoomsNumberBinding.inflate(layoutInflater)
        setContentView(binding.root)

        range = Json.decodeFromString(Json.encodeToString(Application.config.ranges.roomsNumber))
    ...
    private fun initResetListener() {
        binding.reset.setOnClickListener {
            ...
            config.ranges.roomsNumber = Range(1, 20)
    ...
    private fun initApplyListener() {
        binding.apply.setOnClickListener {
            if (range.from !in 1..20 || range.to !in 1..20) {
                Tools.toastMessageLongTime(context, context.getString(R.string.message_rooms_number_range))
            ...
            val config = Application.config
            config.ranges.roomsNumber = range
            Application.config = config
            dismiss()
            context.startActivity(Intent(context, MainActivity::class.java))
        }
